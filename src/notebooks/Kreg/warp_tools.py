...
def get_ref_img_idx(img_f_list, ref_img_name=None):
    ...
    if ref_img_name is None:
        ...
        if os.path.exists(ref_img_name):  
            ref_img_name = valtils.get_name(os.path.split(ref_img_name)[1])

        img_names = []
        for f in img_f_list:
            if os.path.exists(f):
                img_names.append(valtils.get_name(f).lower())
            else:
                img_names.append(f.lower())
        
        # Waiting for a fix in the Valis release, see Issue #170 https://github.com/MathOnco/valis/issues/170
        # img_names = [valtils.get_name(f).lower() for f in img_f_list]
        # ref_escape = re.escape(ref_img_name.lower()) # Escape special characters
