...
def gate_finder(
    ...
    **kwargs,
    ...
    # Add gating layer
    def add_phenotype_layer(adata, gates, phenotype_layer, x, y, viewer, point_size, point_factor):
        cells = gates[gates[phenotype_layer] == 1].index
        coordinates = adata[cells]
        # Flip Y axis if needed
        if flip_y is True:
            coordinates = pd.DataFrame(
                {'y': coordinates.obs[y]*point_factor, 'x': coordinates.obs[x]*point_factor}
            )
        else:
            coordinates = pd.DataFrame(
                {'x': coordinates.obs[x]*point_factor, 'y': coordinates.obs[y]*point_factor}
            )
